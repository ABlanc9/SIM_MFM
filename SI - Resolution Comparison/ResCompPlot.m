...
%     load(QPtFile{i},'xpt','ypt','Imax')
    ...
    IROI=Imax(ROI{i}(3):ROI{i}(4),ROI{i}(1):ROI{i}(2));
    ImaxAdj=Imax(:)-min(IROI(:));
    Ilim=quantile(IROI(:)-min(IROI(:)),.99);
...
%     end
    ...
    Imax2=double(Imax2{1}{1});
    
    IROI2=Imax2((2*ROI{i}(3)):(2*ROI{i}(4)),(2*ROI{i}(1)):(2*ROI{i}(2)));
    ImaxAdj2=Imax2(:)-min(IROI2(:));
    Ilim2=quantile(IROI2(:)-min(IROI2(:)),.99);
    Brightness2=ImaxAdj2/Ilim2*255;
    Brightness2(Brightness2>255)=255;
    Brightness2(Brightness2<0)=0;
    RGB2=uint8(zeros(size(Imax2,1),size(Imax2,2),3));
    RGBTemp2=uint8(zeros(size(Imax2,1),size(Imax2,2)));
    Az2=imresize(Az,2);
    RGB2(:,:,1)=reshape(interp1(AzRef,cmap(:,1),...
        Az2(:)).*Brightness2,size(Imax2,1),size(Imax2,2));
...
%     figure;
    ...
    imshow(RGB,[])
    hold on
    plot([10 41.75]/2*3+ROI{i}(1),[10 10]/2*2+ROI{i}(3),'w','LineWidth',2)
    axis(ROI{i})
    plot(xNew+Cent(1),yNew+Cent(2),'w--','LineWidth',1)
    subplot(4,10,i*2)
    imshow(RGB2,[])
    hold on
    plot([10 41.75]*3+ROI{i}(1)*2,[10 10]*2+ROI{i}(3)*2,'w','LineWidth',2)
    axis(ROI{i}*2)
%     plot((xNew+Cent(1))*2,(yNew+Cent(2))*2,'w--','LineWidth',1)
end
% save('Multi-cell output.mat','Sig2','Sig','peakI','peakI2','Keep','bkrdI','bkrdI2','Loc','Loc2')