...
load('Drift Correction\Background Intensity.mat','I')
load('Timelapses.mat','Timelapse')
load('ROI Selection Process\ROIs.mat','ROI')
...
for iiii=1:length(IndOfInterest)%81
...
%         drawnow
    ...
    if ~isempty(tLim)
        patch(t(tLim([1 2 2 1])),[0 0 1 1],[254 254 225]/255,'FaceAlpha',.5)
        hold on
        Ind=(tLim(1)):tLim(2);
        Ind(Ind==0)=[];
